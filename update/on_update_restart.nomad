job "test" {
  datacenters = ["dc1"]

  group "test" {
    count = 1

    network {
      port "http" {
        to = "8080"
      }
    }

    update {
      health_check = "checks"
    }

    service {
      name = "script-check-svc"
      port = "http"

      check_restart {
        limit           = 2
        ignore_warnings = true
      }

      check {
        name     = "tcp_validate"
        type     = "tcp"
        port     = 6379
        interval = "10s"
        timeout  = "2s"
      }

      check {
        on_update = "ignore_warnings"
        name      = "script-check-script"
        type      = "script"
        command   = "/bin/bash"
        interval  = "5s"
        timeout   = "2s"
        task      = "server"

        args = [
          "-c",
          "/local/ready.sh"
        ]

      }
    }


    task "server" {
      driver = "docker"

      config {
        image = "redis"
      }

      env {
        a = "b"
      }

      # Check script that reports as warning for long enough for deployment to
      # become healthy then errors
      template {
        data = <<EOT
#!/bin/sh

if [ ! -f /tmp/check_1 ]; then touch /tmp/check_1; exit 1; fi
if [ ! -f /tmp/check_2 ]; then touch /tmp/check_2; exit 1; fi
if [ ! -f /tmp/check_3 ]; then touch /tmp/check_3; exit 1; fi
if [ ! -f /tmp/check_4 ]; then touch /tmp/check_4; exit 1; fi
if [ ! -f /tmp/check_5 ]; then touch /tmp/check_5; exit 1; fi
if [ ! -f /tmp/check_6 ]; then touch /tmp/check_6; exit 2; fi
if [ ! -f /tmp/check_7 ]; then touch /tmp/check_7; exit 2; fi
if [ ! -f /tmp/check_8 ]; then touch /tmp/check_8; exit 2; fi
if [ ! -f /tmp/check_9 ]; then touch /tmp/check_9; exit 2; fi


if [ -f /tmp/check_9 ]; then exit 2; fi
EOT

        destination = "local/ready.sh"
        perms       = "777"
      }
    }
  }
}

